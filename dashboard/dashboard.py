...
@app.callback(
    Output("plot", 'figure'),
    [Input("ctrl_time", "value"), Input('interval_update', 'n_intervals')]
)
def update_plot(mins, _):
    """This function creates the plot"""
    data = []
    for psu in PSUS:
        with rcl.pipeline(transaction=True) as pipe:
            pipe.lrange(f"{psu}:iread:val", 0, _BUFFERLEN-1)
            pipe.lrange(f"{psu}:iread:t", 0, _BUFFERLEN-1)
        ...
        logger.info(f"Pulled from {psu}:iread")
